      ...
      <div className={`${s.header} px-3 py-2.5`}>
        <div className="flex items-center justify-between">
          ...
          <span className="text-white/80 text-[10px] font-semibold bg-white/20 rounded px-1.5 py-0.5">
            {(feeRate * 100).toFixed(1).replace('.0', '')}%
          </span>
        </div>
...
// Props:
//   product    : 商品オブジェクト
//   feeRates   : { mercari, yahoo, rakuma, yahuoku } 現在の手数料率
//   onClose    : 閉じるコールバック
//   onSaveRoute: (route) => void
export default function SimulationModal({ product, feeRates, onClose, onSaveRoute }) {
          ...
          {feeRates && feeRates.rakuma !== 0.10 && (
            <div>
              ...
              <span className="text-sm font-bold text-amber-600">
                {(feeRates.rakuma * 100).toFixed(1).replace('.0', '')}%
