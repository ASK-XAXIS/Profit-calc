...
const PLATFORM_STYLE = {
  mercari: {
    ...
    radio:       'accent-red-500',
    editBtn:     'text-red-600 underline decoration-dotted hover:opacity-70',
  },
  yahoo: {
    header:      'bg-purple-500',
    headerText:  'text-white',
    border:      'border-purple-200',
    rowBest:     'bg-purple-50',
    badge:       'bg-purple-100 text-purple-700',
    radio:       'accent-purple-500',
    editBtn:     'text-purple-600 underline decoration-dotted hover:opacity-70',
  },
  rakuma: {
    header:      'bg-pink-500',
    headerText:  'text-white',
  ...
  async function handle() {
    ...
    try {
      ...
      if (price !== null) {
        ...
        window.open(getSearchUrl(productName, platform), '_blank', 'noopener')
      }
    } finally {
      setLoading(false)
    }
...
function PlatformColumn({
  ...
  // sellPrice が外から変わったとき input を追従
  ...
  return (
    <div className={`flex flex-col border ${s.border} rounded-xl overflow-hidden min-w-0`}>
      ...
      <div className={`${s.header} px-3 py-2.5`}>
        <p className={`font-bold text-sm ${s.headerText}`}>{PLATFORM_LABEL[platform]}</p>
        ...
        <div className="flex items-center gap-1 mt-1 flex-wrap">
          ...
          {isEditing ? (
            <div className="flex items-center gap-0.5">
              <input
                ...
                className="w-16 rounded border border-white/50 bg-white/20 px-1.5 py-0.5 text-xs text-white text-right placeholder-white/50 focus:outline-none focus:bg-white/30"
              />
              <span className="text-white/70 text-[10px]">円</span>
            </div>
          ) : (
            <button
              onClick={() => { setInputVal(String(sellPrice)); setIsEditing(true) }}
          ...
          <MarketSearchButton
            ...
            platform={platform}
            onPriceFound={(price) => {
              onSellPriceChange(platform, price)
...
// ── メインモーダル ────────────────────────────────────────
export default function SimulationModal({ product, onClose, onSaveRoute }) {
  ...
  }), [sellPrices, product.buyPrice, packCost, product.thickness])
  ...
  function handleSaveRoute() {
    ...
    onSaveRoute({
      ...
      shipping:     selectedRow.shippingValue,
      shippingFee:  selectedRow.shippingFee,
      sellPrice:    selectedRow.sellPrice,
      profit:       selectedRow.profit,
      serviceLabel: selectedRow.serviceLabel,
      shippingLabel:selectedRow.shippingLabel,
    })
    onClose()
  }
      ...
      <div className="w-full max-w-3xl bg-white rounded-2xl shadow-2xl my-4 overflow-hidden flex flex-col">
        ...
        {(!product.thickness || product.thickness === '') && (
          <div className="px-5 py-2 bg-amber-50 border-b border-amber-200 shrink-0">
            <p className="text-xs text-amber-700">
              ⚠ 厚みが未登録のため全発送方法を表示中。商品編集で厚みを登録すると絞り込まれます。
            </p>
          </div>
        )}
          ...
          <div className="px-5 py-2.5 bg-yellow-50 border-b border-yellow-200 flex items-center gap-2 shrink-0">
            ...
            <div className="min-w-0">
              ...
              <span className="text-xs font-semibold text-yellow-800 truncate block">
                ...
                &ensp;→&ensp;
                <span className="text-sm font-bold">+{bestRow.profit.toLocaleString()}円</span>
              </span>
            </div>
          </div>
        )}
        ...
        <div className="px-5 py-2 bg-blue-50 border-b border-blue-100 shrink-0">
          <p className="text-[10px] text-blue-500">
            💡 ラジオボタンで販売ルートを選択 → 「この販売ルートで登録」で商品情報に保存されます。販売額はプラットフォーム名の下をタップして変更できます。
          </p>
        </div>
        ...
        <div className="px-5 py-4 border-t border-gray-100 flex items-center justify-between gap-3 shrink-0">
          ...
          <div className="min-w-0 flex-1">
            {selectedRow ? (
              <div className="text-xs text-gray-600">
                ...
                {' ／ '}{selectedRow.serviceLabel}
                {' ／ '}{selectedRow.shippingLabel}
                {' '}
                <span className="font-bold text-blue-600">
                  +{selectedRow.profit.toLocaleString()}円
                </span>
              </div>
            ) : (
          ...
          <div className="flex gap-2 shrink-0">
            <button
              onClick={onClose}
              className="rounded-lg px-4 py-2 text-sm font-semibold text-gray-500 bg-gray-100 hover:bg-gray-200 transition"
            >
              閉じる
            </button>
            <button
